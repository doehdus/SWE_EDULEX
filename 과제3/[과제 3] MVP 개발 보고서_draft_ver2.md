...
### 2-4. Product Backlog (v1.5 확정본)

| ID | Title | User Story | Constraints | 우선순위 |
|----|-------|-----------|-------------|---------|
| L00 | 랜딩 페이지 | 비로그인 사용자는 EduLex 소개·기능 안내를 보고 회원가입/로그인으로 진입할 수 있다. | 로그인 상태면 /main 리다이렉트, 정적 콘텐츠 + 라우팅만 | Medium |
| L01 | 회원가입 | 사용자는 이메일 인증, 비밀번호, 닉네임을 입력하여 계정을 생성할 수 있다. | 이메일 인증 필수, 닉네임 중복 불가, star_dust 초기값 0 | High |
| L02 | 사용자 로그인 | 사용자는 이메일/비밀번호 또는 소셜 계정으로 빠르게 접속할 수 있다. | 일반 로그인 + Google OAuth 모두 지원 | High |
| L05 | 관리자 로그인 | 관리자는 별도 계정으로 로그인하여 관리자 전용 페이지에 접근할 수 있다. | users.role DB 조회로 권한 검증 (클라이언트 role 저장 금지) | High |
| H01 | 캐릭터 프리뷰 + 전공/칭호 선택 | 사용자는 메인 화면에서 캐릭터 아이콘·레벨·칭호·전공을 확인하고 전공(최대 2개)을 변경할 수 있다. | MajorContext 전역 공유, 전공 변경 시 N01 단어장 자동 갱신 | Medium |
| H02 | 출석 체크 / 스트릭 | 사용자는 메인 화면에서 하루 1회 출석 버튼을 눌러 연속 출석 일수와 책갈피 보상을 받을 수 있다. | check_attendance RPC 트랜잭션, 7일 캘린더 시각화, 당일 중복 출석 불가 | Medium |
| H04 | PDF → AI 단어장 생성 | 사용자는 PDF를 업로드하여 AI가 자동 생성한 단어장을 즉시 만들 수 있다. | Supabase Edge Function + TF-IDF 로컬 추출 + Groq API (llama-3.3-70b), 단어장 2개 초과 시 생성 차단 | High |
| H05 | 책갈피 실시간 조회 | 사용자는 메인 화면에서 보유 중인 책갈피 수량을 실시간으로 확인할 수 있다. | Supabase Realtime 구독, 본인 row만 구독 (filter 적용) | High |
| N01 | 공식 단어장 조회 | 사용자는 선택 전공을 기반으로 공식 단어장 목록과 단어 카드를 조회할 수 있다. | MajorContext 연동, general_meaning null 시 DOM 렌더링 제외 | High |
| N02 | 나만의 단어장 조회 | 사용자는 PDF→AI로 생성된 단어장 목록과 단어 카드를 조회하고 삭제할 수 있다. | 단어장 최대 2개, 삭제 시 FK CASCADE (user_words 자동 삭제) | High |
| N03 | 테스트 (Lv1) | 사용자는 공식/나만의 단어장을 선택해 Lv1 퀴즈를 풀고 결과와 책갈피 보상을 받을 수 있다. | 뜻→영어 4지선다, 문제 생성·정답 검증 클라이언트, save_quiz_result RPC 트랜잭션, Lv2~4 잠금 표시 | High |
| U04 | 단어장별 학습 진행률 | 사용자는 대시보드에서 단어장별 학습 진행률(%)과 퀴즈 기록을 확인할 수 있다. | word_progress 테이블 기반 완료 단어 수 / 전체 단어 수, quiz_results 기록 조회 | Medium |
| A01 | 공식 단어장 CRUD | 관리자는 공식 단어장 및 단어를 추가·수정·삭제할 수 있다. | RLS role='admin' 검증, 전공별 단어장 목록 관리 | High |
...
### 3-5. Sprint Backlog — 팀 분배 (v1.6 기준)

**Sprint 기간:** 1주 (병렬 개발 — 팀A: FE1+BE1 / 팀B: FE1+BE1+서기(개발 병행))

| 팀 | 개발 단계 | 담당 SBI |
|----|---------|---------|
| **팀A** | 0단계 | SBI-L00 랜딩 페이지 |
| | 1단계 | SBI-L01 회원가입 |
| | 2단계 | SBI-L02 로그인, SBI-L05 관리자 로그인 |
| | 3단계 | SBI-A01 공식단어장 CRUD, **SBI-H04** PDF→AI 생성 |
| | 4단계 | SBI-H05 책갈피 실시간 조회 (`useStarDust` 훅) |
| **팀B** | 1단계 | SBI-H01 캐릭터 + 전공 선택 *(팀A 인증 완료 후 시작)* |
| | 2단계 | SBI-H02 출석/스트릭 (`AttendanceStreak` 컴포넌트 + `useMainPage` 훅), **SBI-N01** 공식단어장 조회 |
| | 3단계 | SBI-N02 나만의 단어장 *(팀A H04 완료 후 연동)* |
| | 4단계 | SBI-N03 테스트(Lv1) (`QuizPage`, `save_quiz_result` RPC), SBI-U04 학습 진행률 (`DashboardPage`, `word_progress` 테이블) |

> **팀A↔팀B 교환 근거 (v1.5):** H04(PDF→AI)는 OpenAI API 독립 작업이므로 팀A가 전담. N01(공식단어장)은 H01 전공 선택 전역 상태(MajorContext)와 공유해야 하므로 같은 팀B가 담당해야 연동 충돌 없음.
...
| v1.6 | L00 랜딩 페이지 추가, N03 클라이언트 문제 생성 방식 확정, U04 word_progress 테이블 기반 진행률 확정 |
...
- **모듈 크기**: 프롬프트로 구현을 지시하거나 수정할 때 반드시 모듈(컴포넌트·함수·훅)의 크기와 책임 범위를 명시 — AI가 하나의 함수/컴포넌트에 모든 로직을 몰아 작성하지 않도록 제약
- **변수 초기화**: 모든 변수는 선언과 동시에 초기값을 명시 — `[제약]` 블록에 "모든 state/변수 선언 시 초기값 명시" 포함
...
어떻게 바꿔달라는지 구체적으로 요청
    ↓
[제약] 추가 — 모듈 크기(분리 기준) + 변수 초기화 명시
```

이 방식을 통해 AI에게 **"무엇을", "왜", "어떻게"** 를 한 번에 전달함으로써 재질문 없이 원하는 결과물로 수렴하는 시간을 단축하였다.

> **수정 지시 시 반드시 포함하는 제약 두 가지**
> - **모듈 크기**: 수정 대상 sector의 책임 범위를 명시하여 AI가 함수/컴포넌트를 불필요하게 비대하게 만들지 않도록 제약 (예: "이 훅은 데이터 페칭 로직만 담당, UI 로직은 포함하지 말 것")
> - **변수 초기화**: 새로 생기거나 수정되는 모든 변수·state는 선언 시 초기값 명시를 요구 (예: "count 같은 숫자 state는 `useState(0)`, 배열은 `useState([])` 형태로")
...
**수정 지시 1차 (sector: AttendanceStreak — 출석 여부 판단):**
```
AttendanceStreak에서 new Date().toLocaleDateString()으로 출석 여부 체크하고 있는데,
서버 시간대랑 다르면 오늘 출석했는데 미출석으로 뜨는 버그 생겨.
클라이언트 날짜 비교 코드 다 지우고, 페이지 진입 시 attendance 테이블에서
오늘 날짜(UTC ISO 날짜)로 SELECT해서 존재 여부로 버튼 상태 결정하게 바꿔줘.
```

**수정 지시 2차 (sector: AttendanceStreak — 출석 완료 피드백 + 7일 캘린더):**
```
AttendanceStreak에서 출석 완료 후에 버튼만 비활성화되고 아무 반응이 없어.
그리고 스트릭 일수만 표시되는데 최근 7일 어느 날 출석했는지 시각적으로 보고 싶어.
출석 성공하면 버튼 텍스트 "오늘 출석 완료 ✓"로 바꾸고 색 전환해줘.
최근 7일 날짜를 가로로 나열하고, 출석한 날은 채워진 원, 아닌 날은 빈 원으로 표시해줘.
```

**수정 지시 3차 (sector: useMainPage — 메인 페이지 데이터 훅 분리):**
```
MainPage에 출석 상태, 스트릭, 단어장 수, 최근 단어장/퀴즈 기록을 다 직접 조회하고 있어서
컴포넌트가 너무 비대해.
useMainPage 훅으로 분리해줘 — 데이터 페칭 로직(출석 여부, 스트릭, wordbookCount, recentWordbook, recentQuiz)만 담당하고
UI 로직은 포함하지 말 것.
모든 state 초기값 명시: checkedToday: useState(false), streak: useState(0), wordbookCount: useState(0)
```

**BE 구현 프롬프트:**
```
[컨텍스트]
기술스택: Supabase (DB + RLS + RPC)
DB 테이블: attendance (id, user_id, date, created_at), users (star_dust)

[구현 대상: SBI-H02 출석 체크 BE]
  1. 출석 기록 INSERT — attendance 테이블 (user_id, date: 오늘 UTC 날짜)
  2. 출석 보상 책갈피 지급 — check_attendance RPC 함수 (트랜잭션):
     - attendance INSERT
     - users.star_dust += 10
  3. 스트릭 계산 — 최근 연속 출석일 수 (attendance 테이블 날짜 연속 확인)

[제약]
- RLS: attendance INSERT는 본인+오늘 날짜만 허용 (중복 출석 방지)
- 책갈피 지급은 check_attendance RPC 단일 함수로 트랜잭션 보장
- 당일 출석 여부 판단: 클라이언트가 attendance 테이블 직접 SELECT (별도 API 없음)
```

**수정 지시 1차 (sector: check_attendance RPC — 중복 출석 방어):**
```
check_attendance RPC에서 오늘 이미 출석한 상태인데도 재호출하면
attendance INSERT 시도하다가 RLS 오류가 터져.
RPC 함수 내부에서 오늘 날짜 출석 기록 존재 여부를 먼저 체크하고,
이미 있으면 { already_attended: true } 반환하게 해줘.
클라이언트에서 버튼 비활성화로 막지만 서버 레벨 이중 방어가 필요해.
```
...
```
[컨텍스트]
기술스택: Supabase Edge Functions (Deno), Groq API (llama-3.3-70b-versatile)
...
환경변수: GROQ_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SUPABASE_ANON_KEY

[구현 대상: SBI-H04 PDF→AI 단어장 생성 BE — 2단계 파이프라인]
  1단계 (로컬): PDF 바이너리에서 BT...ET 블록 파싱으로 텍스트 추출 (외부 라이브러리 없음)
  2단계 (로컬): TF-IDF 알고리즘으로 핵심 단어 50개 선별
    - 문장을 미니 문서로 분할 → 단어별 TF×IDF 점수 계산 → 상위 50개 추출
    - 불용어(stopwords) 목록으로 기능어 사전 제거
  3단계 (Groq API): 선별된 50개 단어 목록을 Groq에 전달 → 단어장 JSON 생성
    - 모델: llama-3.3-70b-versatile, temperature: 0.3, response_format: json_object
    - 출력 형식: { "title": "...", "words": [{ "english", "general_meaning", "major_meaning", "general_example", "major_example" }] }
    - 최대 30개 단어, 전공 용어 우선 선택
  4단계: user_wordbooks + user_words DB 저장

[제약]
- API 키는 환경변수로만 관리 (GROQ_API_KEY)
- PDF 텍스트 추출: 외부 파서 라이브러리 없이 BT/ET 블록 직접 파싱 (Edge Function 환경 제약)
- 단어장 개수 검증: API 레벨에서 2개 초과 시 400 에러 반환
- Groq response_format json_object 사용으로 JSON 파싱 오류 최소화
- 모든 변수 초기값 명시
```

**수정 지시 1차 (sector: Edge Function — OpenAI → Groq 전환):**
```
Edge Function에서 OpenAI API 쓰고 있는데 비용 문제로 Groq로 바꿔줘.
엔드포인트: https://api.groq.com/openai/v1/chat/completions
모델: llama-3.3-70b-versatile
헤더의 Authorization은 Bearer ${GROQ_API_KEY} 방식 그대로.
응답 구조가 OpenAI랑 동일해서 파싱 로직은 유지해도 돼.
환경변수 이름도 OPENAI_API_KEY → GROQ_API_KEY로 바꿔줘.
```

**수정 지시 2차 (sector: Edge Function — PDF 텍스트 추출):**
```
Edge Function에서 pdf-parse 라이브러리 쓰고 있는데 Deno 환경에서 동작을 안 해.
외부 라이브러리 없이 PDF 바이너리를 직접 파싱해줘.
PDF 스펙에서 텍스트는 BT...ET 블록 안에 (text) Tj 또는 [(text)] TJ 형식으로 저장돼.
Uint8Array를 latin1로 디코딩 후 정규식으로 BT...ET 블록 추출 → Tj/TJ 패턴으로 텍스트 수집해줘.
```

**수정 지시 3차 (sector: Edge Function — TF-IDF 단어 선별):**
```
지금 PDF 전체 텍스트를 그대로 Groq에 보내고 있는데
텍스트가 길면 토큰 초과 에러 나고 관련 없는 단어도 많이 섞여.
Groq 호출 전에 TF-IDF로 핵심 단어 50개만 뽑아서 단어 목록만 전달하게 바꿔줘.
문장 단위로 쪼개서 미니 문서로 간주, TF×IDF 점수 계산 후 상위 50개 추출.
불용어 목록(the, a, is 등)도 사전에 필터링해줘.
```
...
**수정 지시 4차 (sector: OfficialWordbookPage — 전체 UI 개선, 디자이너 관점):**
```
[컨텍스트]
참고 사이트: Quizlet, Duolingo, Notion — 학습 웹앱 인기 사이트 디자인 벤치마킹
기존 문제: 사이드바 단조롭고 단어 카드에 계층 구분 없음, 목차에서 원하는 단어 찾기 불편

[구현 대상: OfficialWordbookPage 전체 UI 리디자인]
1. 사이드바 — 전공 태그 뱃지 + 단어장 수 카운터, 선택 시 그라디언트 배경 + 깊이감 있는 그림자
2. 목차 패널 — 실시간 검색 필터(영어/한국어 의미 동시 검색) + 총 단어 수 표시
3. 단어 카드 — 일반 의미/전공 의미를 배경색+테두리가 다른 카드 박스로 구분 (Notion 인용블록 참고)
4. 예문 표시 — 따옴표 인용문 스타일 + 좌측 컬러 바
5. 페이지 전환 — fade + translateY 복합 애니메이션
6. 이전/다음 버튼 — pill 배경 + hover scale 효과
7. 헤더 — 선택된 전공 태그 배지 형태로 표시, 단어장 수 통계 카드
8. 전체 배경 — 단색 → 미묘한 그라디언트 (linear-gradient 160deg)

[제약]
- 기존 MajorContext, Supabase 연동 로직 변경 없음
- Tailwind + 인라인 style 혼용 유지
- 검색 필터는 클라이언트 측 필터링 (API 추가 호출 없음)
- 모듈 크기: WordbookSidebar / TableOfContents / WordPage 컴포넌트 분리 유지
- 모든 state 변수 초기값 명시
```
...
**수정 지시 4차 (sector: MyWordbookPage — 전체 UI 개선, 디자이너 관점):**
```
[컨텍스트]
참고 사이트: Quizlet, Duolingo, Notion — 학습 웹앱 인기 사이트 디자인 벤치마킹
기존 문제: 사이드바에 단어 수 정보 없음, 단어 카드 정보 구분 어려움, 목차 검색 기능 없음

[구현 대상: MyWordbookPage 전체 UI 리디자인]
1. 사이드바 — 각 단어장별 단어 수 뱃지 표시 (Supabase count 쿼리로 사전 로드),
   선택 시 그라디언트 배경 + 그림자, Sparkles 아이콘 + "AI 생성" 레이블
2. 목차 패널 — 실시간 검색 필터(영어/한국어 의미) + 총 단어 수 표시
   active 항목 좌측 컬러 바 강조
3. 단어 카드 — 일반 의미/전공 의미를 배경·테두리 다른 카드 박스로 분리
   예문 따옴표 인용문 스타일
4. 페이지 전환 — fade + translateY 복합 애니메이션
5. 이전/다음 버튼 — pill 배경 + hover scale 효과
6. 헤더 — 총 단어장 수 + 총 단어 수 통계 카드 (우측 상단)
7. 전체 배경 — 단색 → 그라디언트

[제약]
- Supabase 단어 수 조회는 페이지 진입 시 Promise.all로 병렬 처리 (N+1 방지)
- 기존 삭제 모달, PdfUploadBar 연동 로직 변경 없음
- 검색 필터는 클라이언트 측 (API 추가 호출 없음)
- 모든 state 변수 초기값 명시 (wordCounts: useState({}))
```
...
기술스택: React, Tailwind CSS, Supabase
Supabase 클라이언트: src/utils/supabase.js
DB 테이블: official_wordbooks, user_wordbooks, official_words, user_words,
           quiz_results (id, user_id, wordbook_id, wordbook_type, score, total, correct),
           word_progress (id, user_id, wordbook_id, word_id, is_completed)

[구현 대상: SBI-N03 테스트 FE]
  1. 퀴즈 단계 관리: select(단어장 선택) → level(레벨 선택) → quiz(문제 풀기) → result(결과)
  2. 단어장 선택 화면 — 공식/나만의 탭으로 단어장 목록 표시
  3. 레벨 선택 화면 — Lv1(뜻→영어 4지선다) 활성, Lv2~4 잠금 표시
  4. Lv1 퀴즈 UI — 문제 인덱스, 4개 선택지, 선택 표시
  5. 결과 화면 — 정답률 + 정답 수/총 문제 수 + 획득 책갈피 표시

[제약]
- 문제 생성은 클라이언트에서 처리: 단어 목록 로드 후 buildQuestions() 함수로 최대 10문제 생성
  (4지선다: 정답 1개 + 나머지 단어에서 무작위 오답 3개)
- 퀴즈 결과 저장은 save_quiz_result RPC 호출 (트랜잭션으로 quiz_results INSERT + users.star_dust += 보상)
- word_progress 업데이트: 정답 처리된 word_id → is_completed=true upsert
- Lv2~4 버튼은 렌더링하되 disabled 처리 + "준비 중" 뱃지
- 모든 state 초기값 명시: step: useState('select'), answers: useState({}), questions: useState([])
```

**수정 지시 1차 (sector: QuizCard — 보기 선택 피드백):**
```
QuizCard 보기 4개가 그냥 버튼으로만 있어서 내가 뭘 선택했는지 티가 안 나.
선택된 보기는 배경색 + 테두리 강조하고,
다른 거 클릭하면 이전 선택 해제되는 라디오 버튼 동작으로 해줘.
상단에 문제 번호 / 총 문제 수 표시도 추가해줘.
```

**수정 지시 2차 (sector: QuizPage — 결과 저장 RPC):**
```
QuizPage 결과 저장에서 quiz_results INSERT랑 star_dust UPDATE를 별도 쿼리 두 개로 하고 있어.
저장 실패하면 책갈피만 지급되거나 반대 상황 생겨.
save_quiz_result RPC 하나로 묶어줘.
인자: user_id, wordbook_id, wordbook_type, score, total, correct, reward_amount
RPC 내부에서 quiz_results INSERT + users.star_dust += reward_amount 트랜잭션 처리.
```

**수정 지시 3차 (sector: QuizResultPage — 결과 화면 디자인):**
```
QuizResultPage 결과 화면이 정답률 숫자만 있어서 너무 밋밋하고 성취감이 없어.
정답률에 따라 이모지 + 메시지 다르게 해줘 (100%면 완벽해요!, 70% 이상 잘했어요!, 그 이하 다음엔 더 잘할 수 있어요!).
책갈피 획득량은 크게 강조하고 animate-bounce 넣어줘.
다시 테스트하기 버튼이랑 단어장으로 돌아가기 버튼 두 개 추가해줘.
```

**수정 지시 4차 (sector: QuizPage — word_progress 업데이트):**
```
QuizPage에서 정답 처리한 단어를 word_progress 테이블에 저장 안 하고 있어.
DashboardPage에서 진행률 계산할 때 word_progress.is_completed 기준인데 데이터가 없어.
제출 시 정답 처리된 word_id 목록을 word_progress 테이블에 upsert해줘.
(user_id, wordbook_id, word_id, is_completed=true) — 이미 있으면 업데이트.
```
...
[컨텍스트]
기술스택: Supabase (DB + RLS + RPC)
DB 테이블: quiz_results (id, user_id, wordbook_id, wordbook_type, score, total, correct, created_at),
           users (star_dust), word_progress (user_id, wordbook_id, word_id, is_completed)

[구현 대상: SBI-N03 테스트 BE]
  1. save_quiz_result RPC 함수 — 트랜잭션:
     - quiz_results INSERT (user_id, wordbook_id, wordbook_type, score, total, correct)
     - users.star_dust += reward_amount
  2. word_progress upsert — 정답 처리된 단어 학습 완료 기록

[제약]
- 문제 생성·정답 검증은 클라이언트에서 처리 (서버 세션 불필요)
- save_quiz_result는 단일 RPC 트랜잭션 (실패 시 전체 롤백)
- wordbook_type은 'official' | 'user' 중 하나 (RLS 검증 활용)
```

**수정 지시 1차 (sector: save_quiz_result RPC — wordbook 존재 검증):**
```
save_quiz_result RPC에서 wordbook_id 유효성 검증이 없어.
존재하지 않는 wordbook_id로도 결과가 저장돼버려.
RPC 내부에서 wordbook_type이 'official'이면 official_wordbooks,
'user'이면 user_wordbooks에서 해당 id 존재 여부 확인 후 없으면 에러 반환하게 해줘.
```

---

#### SBI-U04 | 단어장별 학습 진행률 (DashboardPage)
...
Supabase 클라이언트: src/utils/supabase.js
DB 테이블: official_wordbooks, user_wordbooks, official_words, user_words,
           word_progress (user_id, wordbook_id, word_id, is_completed),
           quiz_results (user_id, wordbook_id, wordbook_type, score, total, correct, created_at)

[구현 대상: SBI-U04 학습 진행률 대시보드 FE]
  1. 공식/나만의 단어장 목록 + 각 단어장별 학습 진행률 카드
     - 진행률 = (word_progress.is_completed=true 단어 수 / 전체 단어 수) × 100%
     - 프로그레스 바 시각화
  2. 퀴즈 기록 목록 (quiz_results) — 날짜, 단어장명, 점수 표시

[제약]
- 단어장별 전체 단어 수 조회: official_words / user_words select count exact
- 완료 단어 수 조회: word_progress select count (user_id + wordbook_id + is_completed=true)
- 단어장·word_progress 병렬 조회 (Promise.all) — N+1 방지
- 모든 state 초기값 명시: progressData: useState([]), quizHistory: useState([])
```

**수정 지시 1차 (sector: DashboardPage — 로딩 성능):**
```
DashboardPage에서 단어장 하나씩 순차로 word_progress 조회하고 있어서
단어장 많을수록 로딩이 느려.
Promise.all로 모든 단어장의 word_progress를 병렬 조회하게 바꿔줘.
단어장 수 쿼리도 동시에 처리해서 전체 로딩 한 번에 끝내줘.
```

**수정 지시 2차 (sector: DashboardPage — 퀴즈 기록 표시):**
```
DashboardPage에 학습 진행률만 있고 퀴즈 기록이 없어.
언제 어떤 단어장을 테스트했는지 확인하고 싶어.
quiz_results에서 최근 10개 기록 가져와서 날짜·단어장명·정답수/총문제수·점수% 형식으로 목록 추가해줘.
단어장명은 wordbook_id로 official_wordbooks / user_wordbooks 조인해서 가져와줘.
```

**BE:** Supabase RLS — word_progress 본인 row만 SELECT/UPSERT 허용 확인 후 통과
...
3. 컴포넌트 분리 기준 위반 (너무 큰 컴포넌트) 확인 — 모듈 책임이 하나를 넘으면 분리 제안
4. 보안 제약 위반 여부 확인 (클라이언트 role/정답 저장 등)
5. 초기값 없이 선언된 변수·state 확인 — 미초기화 변수는 반드시 초기값 추가
...
| SBI | 기능명 | 주요 구현 파일 | AC 체크리스트 | DoD 완료 |
|-----|--------|-------------|-------------|---------|
| L00 | 랜딩 페이지 | LandingPage.jsx | ✅ | ✅ |
| L01 | 회원가입 (이메일 인증) | SignupPage.jsx | ✅ | ✅ |
| L02 | 로그인 (일반 + Google OAuth) | LoginPage.jsx | ✅ | ✅ |
| L05 | 관리자 로그인 및 권한 분리 | AdminLoginPage.jsx, ProtectedRoute.jsx | ✅ | ✅ |
| H01 | 캐릭터 프리뷰 + 전공 선택 | CharacterPreview.jsx, MajorContext.jsx | ✅ | ✅ |
| H02 | 출석 체크 / 스트릭 + 책갈피 보상 | AttendanceStreak.jsx, useMainPage.js, check_attendance RPC | ✅ | ✅ |
| H04 | PDF → AI 단어장 생성 | PdfUploadBar.jsx, Edge Function create-wordbook-from-pdf (TF-IDF + Groq llama-3.3-70b) | ✅ | ✅ |
| H05 | 책갈피 실시간 조회 | useStarDust.js (Supabase Realtime) | ✅ | ✅ |
| N01 | 공식 단어장 조회 (전공 연동 + 검색) | OfficialWordbookPage.jsx | ✅ | ✅ |
| N02 | 나만의 단어장 조회 + 삭제 | MyWordbookPage.jsx | ✅ | ✅ |
| N03 | 테스트 Lv1 + 결과 저장 + 보상 | QuizPage.jsx, save_quiz_result RPC, word_progress upsert | ✅ | ✅ |
| U04 | 단어장별 학습 진행률 + 퀴즈 기록 | DashboardPage.jsx, word_progress 테이블 | ✅ | ✅ |
| A01 | 공식 단어장 CRUD | AdminWordbookPage.jsx | ✅ | ✅ |
...
### 이슈 항목: H04 PDF 추출 기능 — Sprint 중 문제 발생 후 완료

Sprint 진행 중 **PDF 텍스트 추출 방식의 기술적 장벽**을 확인하였고, 팀 토의를 통해 방향을 전환하여 최종 완료하였다.

#### 팀 토의 과정

1. **문제 발생**: PDF → AI 단어장 생성(H04) 중 Deno(Edge Function) 환경에서 외부 PDF 파서 라이브러리(`pdf-parse` 등)가 동작하지 않음을 확인
2. **MVP 포함 여부 결정**: 이 기능이 다른 영어 단어 웹사이트와의 **차별점**이라는 점을 팀원 전원이 동의 — 포기하지 않고 해결책 탐색
3. **기술 방향 전환**:
   - PDF 파서: 외부 라이브러리 없이 **BT/ET 블록 직접 파싱** 방식으로 전환
   - AI API: 비용 문제로 **OpenAI → Groq (llama-3.3-70b)** 전환
   - 토큰 최적화: PDF 전체 텍스트 전달 → **TF-IDF 로컬 선별 후 단어 목록만** Groq에 전달
4. **완료**: 5명 전원이 각자 구현 방안을 조사·비교 후 최종 구현 완료

> 칸반 보드를 통해 H04 진행이 막혀있음을 팀 전체가 인지하고 있었기 때문에, 빠르게 전원이 투입하여 해결할 수 있었다.
...
| H04 초기 방향 오판 | 외부 PDF 파서 라이브러리가 Deno Edge Function 환경에서 동작하지 않는다는 제약을 초기에 충분히 검토하지 않아 재작업 발생 |
...
| 외부 라이브러리 사용 전 실행 환경(Deno/Node) 호환성 사전 검토를 개발 착수 체크리스트에 추가 | 팀A | 다음 스프린트 시작 전 |

> **합의 사항:** 시험 기간 중 조급하게 처리한 부분은 시험 종료 후 시간을 투자하여 재정비하기로 팀 전원 빠른 합의 완료. H04는 BT/ET 직접 파싱 + Groq 전환으로 최종 완료.
