    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: `1px solid ${LIB.parchmentDark}` }}>
          ...
          <button onClick={() => setSidebarOpen(false)} className="text-2xl leading-none transition hover:opacity-50" style={{ color: LIB.inkLight }}>×</button>
        </div>
          ...
          {MENU_ITEMS.map(({ path, label, icon }) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold relative overflow-hidden"
                style={{
                  background: isActive ? LIB.wood : 'transparent',
                  color: isActive ? LIB.parchment : LIB.inkMid,
                }}
                onMouseEnter={e => { if (!isActive) { e.currentTarget.style.background = LIB.parchmentDark; e.currentTarget.style.transform = 'translateX(4px)' } }}
                onMouseLeave={e => { if (!isActive) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.transform = 'translateX(0)' } }}
              >
                {isActive && <div className="absolute left-0 top-2 bottom-2 w-1 rounded-r-full" style={{ background: LIB.gold }} />}
                {icon}
                {label}
              </Link>
            )
