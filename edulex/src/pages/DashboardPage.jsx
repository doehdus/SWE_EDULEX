...
function ProgressBar({ percent }) {
  return (
    <div className="w-full bg-gray-100 rounded-full h-2.5">
      <div className="bg-[#7c3aed] h-2.5 rounded-full transition-all duration-500" style={{ width: `${percent}%` }} />
    </div>
  )
}
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {loading ? <p className="text-gray-400 text-sm">불러오는 중...</p>
        : progressList.length === 0 ? <div className="bg-white rounded-2xl p-10 text-center text-gray-400"><p>단어장이 없습니다.</p></div>
        : <div className="space-y-4">
            {progressList.map(wb => (
              <div key={wb.id} className="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
                ...
                <p className="text-xs text-gray-400 mt-1.5">{wb.completed} / {wb.total} 단어 완료</p>
              </div>
            ))}
          </div>
      }
