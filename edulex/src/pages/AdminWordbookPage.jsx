...
const NAV_SECTIONS = [
  {
    section: '단어장',
    items: [
      { id: 'wordbook', label: '공식 단어장 관리', icon: '📚', badge: 'A01' },
    ],
  },
  {
    section: '사용자',
  ...
  },
  {
    section: '커뮤니티',
    items: [
      { id: 'qna', label: 'Q&A 게시판 답변', icon: '💬', badge: 'A04' },
    ],
  },
  {
    section: '모니터링',
...
function WordbookPanel() {
  ...
  const openAddWb = () => { setWbForm({ title: '', major: activeMajor }); setModal('addWb') }
  ...
  const submitWb = async (e) => {
    ...
    setModal(null); fetchWordbooks(activeMajor); fetchWbCounts()
  }

  const confirmDeleteWb = (wb) => { setModalTarget(wb); setModal('deleteWb') }
  ...
  const saveWord = async (form) => {
    ...
    setModal(null); fetchWords(activeWb.id)
  }

  const confirmDeleteWord = (word) => { setModalTarget(word); setModal('deleteWord') }
  ...
  return (
    <div className="flex flex-1 min-h-0">
      ...
      <aside className="w-60 bg-white border-r border-gray-200 flex flex-col shrink-0">
        {!activeMajor ? (
          <div className="flex-1 flex items-center justify-center text-gray-300 text-sm">전공을 선택하세요</div>
        ) : (
          <>
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-700">{activeMajor}</p>
              <button onClick={openAddWb} className="text-xs bg-violet-600 text-white px-2.5 py-1 rounded-lg hover:bg-violet-700 transition">+ 추가</button>
            </div>
            <ul className="flex-1 overflow-y-auto py-2">
              {wordbooks.length === 0 ? (
                ...
                <li key={wb.id}>
                  ...
                  >
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 shrink-0 ml-2" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => confirmDeleteWb(wb)} className="text-xs text-red-400 hover:text-red-600 px-1" title="삭제">✕</button>
                    </div>
                  </div>
                </li>
              ))}
      ...
      <main className="flex-1 overflow-y-auto p-6">
        {!activeWb ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-300 gap-2">
            ...
            <p className="text-sm">{activeMajor ? '단어장을 선택하세요' : '전공 카테고리를 먼저 선택하세요'}</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={openAddWord} className="bg-violet-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-violet-700 transition">+ 단어 추가</button>
            </div>
                    ...
                    <tr key={word.id} className="hover:bg-gray-50 group">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition">
                          ...
                          <button onClick={() => confirmDeleteWord(word)} className="text-xs text-red-400 hover:text-red-600 font-medium">삭제</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {/* 모달들 */}
      {modal === 'addWb' && (
        <Modal title="단어장 추가" onClose={() => setModal(null)}>
          <form onSubmit={submitWb} className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block">단어장 제목 *</label>
              <input value={wbForm.title} onChange={e => setWbForm(f => ({ ...f, title: e.target.value }))} required placeholder="e.g. 컴퓨터과학 심화 어휘"
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block">전공 *</label>
              <select value={wbForm.major} onChange={e => setWbForm(f => ({ ...f, major: e.target.value }))}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
                {MAJORS.map(m => <option key={m}>{m}</option>)}
              </select>
            </div>
            <div className="flex gap-2 pt-1">
              <button type="button" onClick={() => setModal(null)} className="flex-1 py-2 text-sm text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">취소</button>
              <button type="submit" className="flex-1 py-2 text-sm text-white bg-violet-600 rounded-lg hover:bg-violet-700">저장</button>
            </div>
          </form>
        </Modal>
      ...
      {modal === 'editWb' && (
        <Modal title="단어장 수정" onClose={() => setModal(null)}>
          <form onSubmit={submitWb} className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block">단어장 제목 *</label>
              <input value={wbForm.title} onChange={e => setWbForm(f => ({ ...f, title: e.target.value }))} required
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block">전공 *</label>
            ...
            <div className="flex gap-2 pt-1">
              <button type="button" onClick={() => setModal(null)} className="flex-1 py-2 text-sm text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">취소</button>
              <button type="submit" className="flex-1 py-2 text-sm text-white bg-violet-600 rounded-lg hover:bg-violet-700">수정 완료</button>
            </div>
          </form>
        </Modal>
      )}
      {modal === 'deleteWb' && (
        <Modal title="단어장 삭제" onClose={() => setModal(null)}>
          <p className="text-sm text-gray-600 mb-1"><strong className="text-gray-900">"{modalTarget?.title}"</strong> 단어장을 삭제하시겠습니까?</p>
          <p className="text-xs text-red-400 mb-5">단어장 안의 모든 단어도 함께 삭제됩니다.</p>
          <div className="flex gap-2">
            <button onClick={() => setModal(null)} className="flex-1 py-2 text-sm text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">취소</button>
            <button onClick={deleteWb} className="flex-1 py-2 text-sm text-white bg-red-500 rounded-lg hover:bg-red-600">삭제</button>
          </div>
        </Modal>
      )}
      {modal === 'addWord' && (
      ...
      {modal === 'deleteWord' && (
        <Modal title="단어 삭제" onClose={() => setModal(null)}>
          <p className="text-sm text-gray-600 mb-5"><strong className="text-gray-900">"{modalTarget?.english}"</strong> 단어를 삭제하시겠습니까?</p>
          <div className="flex gap-2">
            <button onClick={() => setModal(null)} className="flex-1 py-2 text-sm text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">취소</button>
            <button onClick={deleteWord} className="flex-1 py-2 text-sm text-white bg-red-500 rounded-lg hover:bg-red-600">삭제</button>
          </div>
        </Modal>
      )}
    </div>
...
export default function AdminWordbookPage() {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'users':
        return <ComingSoon icon="👥" title="유저 목록 조회" badge="A02" description="닉네임·이메일 기준으로 전체 유저를 검색하고 조회합니다." />
      case 'user-detail':
        return <ComingSoon icon="🪪" title="유저 상세 정보" badge="A03" description="가입 정보(닉네임, 이메일 등) 상세 확인 및 관리." />
      case 'qna':
        return <ComingSoon icon="💬" title="Q&A 게시판 답변" badge="A04" description="사용자 문의에 답변을 작성하고 완료 상태를 관리합니다." />
      case 'db-usage':
        return <ComingSoon icon="🗄️" title="DB 사용량 모니터링" badge="A05" description="Supabase DB 사용량 지표를 실시간으로 확인합니다." />
      case 'stats':
        return <ComingSoon icon="📊" title="전공별 사용자 현황" badge="A06" description="전공별 가입 인원수를 차트로 시각화합니다." />
      default:
        return null
    }
  }
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex flex-1 min-h-0 flex-col">
          ...
          <div className="bg-white border-b border-gray-100 px-6 py-3 shrink-0">
            {NAV_SECTIONS.flatMap(s => s.items).filter(i => i.id === activeMenu).map(item => (
              <div key={item.id} className="flex items-center gap-2">
                <span className="text-lg">{item.icon}</span>
                <h1 className="font-bold text-gray-800">{item.label}</h1>
                <span className="ml-1 text-xs bg-violet-100 text-violet-600 font-bold px-2 py-0.5 rounded-full">{item.badge}</span>
              </div>
            ))}
          </div>
