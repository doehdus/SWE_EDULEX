...
const NAV_SECTIONS = [
  { section: '단어장', items: [{ id: 'wordbook', label: '공식 단어장 관리', icon: '📚', badge: 'A01' }] },
  { section: '사용자', items: [
    { id: 'users',       label: '유저 목록 조회',  icon: '👥', badge: 'A02' },
    { id: 'user-detail', label: '유저 상세 정보',  icon: '🪪', badge: 'A03' },
  ]},
  { section: '커뮤니티', items: [{ id: 'qna', label: 'Q&A 게시판 답변', icon: '💬', badge: 'A04' }] },
  { section: '모니터링', items: [
    ...
    <Modal title={title} onClose={onClose}>
      <form onSubmit={onSubmit} className="space-y-3">
        <div>
          ...
          <input value={form.title} onChange={e => onChangeTitle(e.target.value)} required placeholder="e.g. 컴퓨터과학 심화 어휘"
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400" />
        </div>
        <div>
          <label className="text-xs font-semibold text-gray-500 mb-1 block">전공 *</label>
          <select value={form.major} onChange={e => onChangeMajor(e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
...
function WordbookPanel() {
  ...
  const fetchWordbooks = async (major) => {
    const { data } = await supabase.from('official_wordbooks').select('*').eq('major', major).order('created_at', { ascending: false })
    setWordbooks(data ?? [])
    setActiveWb(null)
    setWords([])
  }
  ...
  const fetchWords = async (wordbookId) => {
    const { data } = await supabase.from('official_words').select('*').eq('wordbook_id', wordbookId).order('created_at', { ascending: true })
    setWords(data ?? [])
  }
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <nav className="w-48 bg-white border-r border-gray-200 flex flex-col shrink-0">
        ...
        <ul className="flex-1 overflow-y-auto py-2">
          {MAJORS.map(major => (
            <li key={major}>
              <button onClick={() => selectMajor(major)}
                className={`w-full text-left px-4 py-2.5 flex items-center justify-between text-sm transition
                  ${activeMajor === major ? 'bg-violet-50 text-violet-700 font-semibold border-r-2 border-violet-600' : 'text-gray-600 hover:bg-gray-50'}`}>
                <span>{major}</span>
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${activeMajor === major ? 'bg-violet-100 text-violet-600' : 'bg-gray-100 text-gray-400'}`}>
                  {wbCounts[major] ?? 0}
                </span>
              </button>
            </li>
      ...
      <aside className="w-60 bg-white border-r border-gray-200 flex flex-col shrink-0">
        {!activeMajor ? <EmptyState message="전공을 선택하세요" /> : (
          <>
            ...
            <ul className="flex-1 overflow-y-auto py-2">
              {wordbooks.length === 0
                ...
                : wordbooks.map(wb => (
                  <li key={wb.id}>
                    <div onClick={() => { setActiveWb(wb); fetchWords(wb.id) }}
                      ...
                      <p className={`text-sm font-medium truncate ${activeWb?.id === wb.id ? 'text-violet-700' : 'text-gray-700'}`}>{wb.title}</p>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 shrink-0 ml-2" onClick={e => e.stopPropagation()}>
                        <button onClick={() => openEditWb(wb)} className="text-xs text-violet-500 hover:text-violet-700 px-1">✎</button>
                        <button onClick={() => { setModalTarget(wb); setModal('deleteWb') }} className="text-xs text-red-400 hover:text-red-600 px-1">✕</button>
                      </div>
                    </div>
                  </li>
                ))}
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {words.length === 0
                    ? <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-300 text-sm">단어가 없습니다. <strong>+ 단어 추가</strong> 버튼을 눌러 추가하세요.</td></tr>
                    : words.map(word => (
      ...
      {(modal === 'addWb' || modal === 'editWb') && (
        <WordbookFormModal
          ...
          form={wbForm} onChangeTitle={setWbField('title')} onChangeMajor={setWbField('major')}
          onSubmit={submitWb} onClose={() => setModal(null)}
          submitLabel={modal === 'addWb' ? '저장' : '수정 완료'}
        />
      )}
      {modal === 'deleteWb' && (
        <ConfirmModal title="단어장 삭제"
          description={<><strong className="text-gray-900">"{modalTarget?.title}"</strong> 단어장을 삭제하시겠습니까?</>}
          warning="단어장 안의 모든 단어도 함께 삭제됩니다."
          onConfirm={deleteWb} onCancel={() => setModal(null)} />
      )}
      {modal === 'addWord' && (
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1 min-h-0">
                ...
                {items.map(({ id, label, icon, badge }) => (
                  <button key={id} onClick={() => setActiveMenu(id)}
                    className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm transition
                      ${activeMenu === id ? 'bg-violet-50 text-violet-700 font-semibold border-r-2 border-violet-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                    <span className="text-base">{icon}</span>
                    <span className="flex-1 text-left leading-tight">{label}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${activeMenu === id ? 'bg-violet-100 text-violet-500' : 'bg-gray-100 text-gray-400'}`}>{badge}</span>
                  </button>
                ))}
              </div>
            ))}
        ...
        <div className="flex flex-1 min-h-0 flex-col">
          ...
          <div className="flex flex-1 min-h-0">{renderContent()}</div>
        </div>
      </div>
    </div>
  )
