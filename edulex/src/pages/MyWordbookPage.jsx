    ...
    <button
      ...
      style={{
        ...
        background: isSelected
          ...
          : hovered ? `linear-gradient(135deg, ${color.accent}55, #fff 60%)` : '#fff',
      ...
      className="w-full text-left px-4 py-3.5 rounded-r-2xl rounded-l-sm text-sm border-l-4 relative overflow-hidden select-none group"
    >
      <div className="flex items-start justify-between relative">
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <p className="text-xs" style={{ color: isSelected ? 'rgba(255,255,255,0.7)' : '#8b6e4e' }}>AI 생성</p>
            {count !== null && (
...
function WordModal({ word, idx, total, color, pageFlip, flipDir, onPrev, onNext, onClose }) {
  useEffect(() => {
    const handler = (e) => {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fffdf7', boxShadow: '0 32px 80px rgba(0,0,0,0.35)', maxHeight: '85vh' }}
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-xs font-mono font-semibold" style={{ color: '#b09070' }}>{idx + 1} / {total}</span>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="h-1 rounded-full mt-3" style={{
...
export default function MyWordbookPage() {
  ...
  const fetchWordbooks = async () => {
    const { data } = await supabase
      .from('user_wordbooks').select('*').eq('user_id', user.id).order('created_at', { ascending: false })
    const wbs = data ?? []
    setWordbooks(wbs)
    if (wbs.length > 0) {
      const counts = {}
      await Promise.all(wbs.map(async wb => {
        const { count } = await supabase
          .from('user_words').select('*', { count: 'exact', head: true }).eq('wordbook_id', wb.id)
  ...
  return (
    ...
    >
      {/* 헤더 + PDF 업로드 */}
      <div className="shrink-0 max-w-6xl w-full mx-auto px-6 pt-5 pb-3">
      ...
      {modalOpen && (
        <WordModal
          word={selectedWord} idx={selectedIdx} total={words.length}
          color={selectedWbColor} pageFlip={pageFlip} flipDir={flipDir}
